...
class ExchangeService
{
    ...
    */
    ...
    public function deleteOne()
    {
        ...
        $obj = ExchangeQuery::create()->findPk(json_decode($this->request['i']));


            if($obj->countAssetExchanges()){
                $error = handleNotOkResponse(_("This entry cannot be deleted. It is in use in ")." 'Wallet'. ", '', true,'Exchange'); die( $error['onReadyJs'] );
            }
            if($obj->countTrades()){
                $error = handleNotOkResponse(_("This entry cannot be deleted. It is in use in ")." 'Trade'. ", '', true,'Exchange'); die( $error['onReadyJs'] );
            }
    ...
    public function saveUpdate()
    {
        ...
        $data['i'] = ( $data['IdExchange'] ) ? $data['IdExchange'] : $this->request['i'];
        $data['ip'] = urldecode($this->request['data']['ip']);
        $data['pc'] = urldecode($this->request['data']['pc']);
        $this->Exchange['request'] = $this->request;

        if(!empty($data['i'])) {
            ## Save

            $e = $this->Form->setUpdateDefaultsExchange($data);
            ...
            $e = $this->Form->setCreateDefaultsExchange($data);
    ...
    */
    ...
    {
        $this->Exchange['request'] = $this->request;
        $this->Exchange['parentId'] = $this->request['data']['ip'];
