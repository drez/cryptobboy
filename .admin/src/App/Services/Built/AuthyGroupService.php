    ...
    */
    ...
    public function deleteOne()
    {
        ...
        $obj = AuthyGroupQuery::create()->findPk(json_decode($this->request['i']));
            ...
            if($obj->countAssets()){
                $error = handleNotOkResponse(_("This entry cannot be deleted. It is in use in ")." 'Asset'. ", '', true,'Group'); die( $error['onReadyJs'] );
            }
            if($obj->countAssetExchanges()){
                $error = handleNotOkResponse(_("This entry cannot be deleted. It is in use in ")." 'Wallet'. ", '', true,'Group'); die( $error['onReadyJs'] );
            }
            if($obj->countTrades()){
                $error = handleNotOkResponse(_("This entry cannot be deleted. It is in use in ")." 'Trade'. ", '', true,'Group'); die( $error['onReadyJs'] );
            }
            if($obj->countExchanges()){
                $error = handleNotOkResponse(_("This entry cannot be deleted. It is in use in ")." 'Exchange'. ", '', true,'Group'); die( $error['onReadyJs'] );
            }
            if($obj->countTokens()){
                $error = handleNotOkResponse(_("This entry cannot be deleted. It is in use in ")." 'Token'. ", '', true,'Group'); die( $error['onReadyJs'] );
            }
            if($obj->countSymbols()){
