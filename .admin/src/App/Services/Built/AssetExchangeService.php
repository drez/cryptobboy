...
class AssetExchangeService
{
    ...
    */
    public function getApiResponse()
    {
        ...
        $Api = new Api('AssetExchange', $this);
    ...
    public function deleteOne()
    {
        ...
        $obj = AssetExchangeQuery::create()->findPk(json_decode($this->request['i']));
    ...
    public function saveUpdate()
    {
        ...
        $data['i'] = ( $data['IdAssetExchange'] ) ? $data['IdAssetExchange'] : $this->request['i'];
        $data['ip'] = urldecode($this->request['data']['ip']);
        $data['pc'] = urldecode($this->request['data']['pc']);
        $this->AssetExchange['request'] = $this->request;

        if(!empty($data['i'])) {
            ## Save

            $e = $this->Form->setUpdateDefaultsAssetExchange($data);
            ...
            $e = $this->Form->setCreateDefaultsAssetExchange($data);
            ...
            if ($e->validate() && !$extValidationErr) {
                ...
                $data['IdAsset'] = json_encode($e->getIdAsset());
            }else{
                $PropelErrorHandler = new PropelErrorHandler($e, $this->request['data']['ui'],_('Form field'), $extValidationErr, $this->virtualClassName);
                $error = $PropelErrorHandler->getValidationErrors();
            }
    ...
    */
    ...
    {
        $this->AssetExchange['request'] = $this->request;
        $this->AssetExchange['parentId'] = $this->request['data']['ip'];

            if($this->request['data']['ip']){
                $relData['IdAsset'] = $this->request['data']['ip'];
